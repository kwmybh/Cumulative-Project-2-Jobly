...
class Company {
	...
	static async findAll(filterParams = []) {
		const filter =
			Object.keys(filterParams).length === 0
				? ''
				: Company.buildWhereClause(filterParams);
	...
	static async get(handle) {
		const companyRes = await db.query(
			`SELECT DISTINCT c.handle, j.title AS 'jobTitle',j.id AS 'jobId', c.name, c.description, c.num_employess AS 'numEmployees', c.logo_Url AS 'logoUrl'
       FROM companies c
      LEFT JOIN jobs j
	...
	static buildWhereClause(filterParams) {
		// takes in object with optional query
		// string parameters and returns a dynamic WHERE clause to add to SQL query
		const {
			name = null,
			minEmployess = null,
			maxEmployees = null,
		...
		const clauses = {
			...
			minEmployess: `num_employees >=${minEmployess}`,
		...
		const selectStatement = Object.entries(filterParams)
			.filter(([key, val]) => val !== null)
			.map(([key, val]) =>
				['minEmployees', 'maxEmployees'].includes(key) ? 'num_employees' : key
			);
		return {
			selectStatement: selectStatement.join(','),
			whereStatement: returnArray.join('AND'),
		};
	}
}
